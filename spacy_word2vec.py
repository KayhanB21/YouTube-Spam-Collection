...
class SpacyWord2Vec:
    ...
    def calc_avg_vector(self):
        ...
        self.target = self.df['CLASS'].values

    def split_train_val_test(self):
        self._split_avg_vect()
        self._split_avg_vect_tfidf()

    def _create_spacy_doc_list(self):
        nlp = spacy.load(settings.modeling.spacy.model, disable=['parser', 'ner'])
        self.docs = list(nlp.pipe(self.df['CONTENT_EDITED'],
    ...
    def _get_avg_vector(self, ):
        ...
        self.avg_vect = np.array(avg_vect)
        """ 
    ...
    def _get_avg_tfidf_vector(self):
        ...
        transformed_documents = tfidf_vectorizer.fit_transform(self.df['CONTENT_EDITED'])
        ...
        self.avg_vect_tfidf = np.array(avg_vect_tfidf)
        """
    ...
    def _split_avg_vect(self):
        x_train, x_test, y_train, y_test, inx_train, inx_test = train_test_split(self.avg_vect,
                                                                                 self.target,
                                                                                 [*range(
                                                                                     len(self.avg_vect))],
        ...
        x_val, x_test, y_val, y_test, inx_val, inx_test = train_test_split(x_test,
                                                                           ...
                                                                           test_size=settings.modeling.split.test / (
                                                                                   ...
                                                                                   settings.modeling.split.test))
        logger.info(f"avg_dataset info, "
                    f"{x_train.shape=}, "
                    f"{y_train.shape=}, "
                    f"{len(inx_train)=}, "
                    f"{x_val.shape=}, "
                    ...
                    f"{y_test.shape=}, "
                    f"{len(inx_test)=}")
        self.avg_vect_dataset = {'x_train': x_train,
                                 'y_train': y_train,
                                 'inx_train': inx_train,
                                 'x_val': x_val,
                                 'y_val': y_val,
    ...
    def _split_avg_vect_tfidf(self):
        x_train, x_test, y_train, y_test, inx_train, inx_test = train_test_split(self.avg_vect_tfidf,
                                                                                 self.target,
                                                                                 [*range(
                                                                                     len(self.avg_vect_tfidf))],
                                                                                 test_size=settings.modeling.split.val +
                                                                                           settings.modeling.split.test)
        x_val, x_test, y_val, y_test, inx_val, inx_test = train_test_split(x_test,
                                                                           y_test,
                                                                           ...
                                                                           test_size=settings.modeling.split.test / (
                                                                                   settings.modeling.split.val +
                                                                                   settings.modeling.split.test))
        logger.info(f"tfidf_dataset info, "
                    f"{x_train.shape=}, "
                    f"{y_train.shape=}, "
                    f"{len(inx_train)=}, "
                    f"{x_val.shape=}, "
                    ...
                    f"{y_test.shape=}, "
                    f"{len(inx_test)=}")
        self.avg_vect_tfidf_dataset = {'x_train': x_train,
                                       'y_train': y_train,
                                       'inx_train': inx_train,
                                       'x_val': x_val,
                                       'y_val': y_val,
